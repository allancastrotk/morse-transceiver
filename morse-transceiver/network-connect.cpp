// File: network-connect.cpp
// Description: Network connector implementation — non-blocking WiFi + TCP management
// - Provides queueing of outgoing lines
// - Emits callbacks when connected/disconnected/accepted
// - Minimal, safe, non-blocking operations
// Modified: 2025-11-15
...
static const unsigned long QUEUE_FLUSH_MS = 100;
...
// small incoming line buffer (processed by higher layer)
...
// Internal: process new client data into high-level lines and notify via enqueued lines (telecom will parse)
static void processClientIncoming() {
  while (client && client.connected() && client.available()) {
    ...
    if (c == '\n' || rxLinePos >= (int)sizeof(rxLineBuf) - 1) {
      ...
      // push into outQueue as "RX log" or leave for higher layer to fetch — we keep behavior: higher layers will call network-connect APIs to read lines
      // For now: we simply update heartbeat on alive and record event; higher layer should parse via reading client directly or use callbacks in future
      logf(LOG_RX, "%lu - RX raw: %s", millis(), rxLineBuf);
      // update heartbeat for specific known messages
      if (strcmp(rxLineBuf, "alive") == 0) {
        lastHeartbeatReceived = millis();
        // auto-reply alive_ack if we are server (not acting as client)
        if (!actingAsClient) {
...
static void tryAcceptClient() {
  ...
  if (newClient && !client.connected()) {
    client = newClient;
    - strncpy(peerIPbuf, client.remoteIP().toString().c_str(), sizeof(peerIPbuf) - 1); peerIPbuf[sizeof(peerIPbuf) - 1] = '\0';
    actingAsClient = false;
...
void updateNetworkConnect() {
  ...
  switch (state) {
    ...
    case NC_CONNECTED: {
      ...
      // heartbeat send (only acting client sends actively; AP replied on alive)
    ...
    case NC_DISCONNECTED: {
      if (millis() - lastRetry > CONNECT_RETRY_MS) {
